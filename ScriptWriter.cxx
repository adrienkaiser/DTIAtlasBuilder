...
//////////Scale Levels:
	...
	std::ostringstream outSLit;
	outSLit << m_AtlasWerksScaleLevels[0][1];
	std::string SLit_str = outSLit.str();
	Script = Script + "NbIter = [" + SLit_str;
	for (unsigned int i=1;i<m_AtlasWerksScaleLevels.size();i++) 
	{
		std::ostringstream outSLit;
		outSLit << m_AtlasWerksScaleLevels[i][1];
		std::string SLit_str = outSLit.str();
		Script = Script + ", " + SLit_str;
	}	
	Script = Script+ "]\n\n";
...
//////////AtlasWerks Command:
	...
	Script = Script + "while scale < len(ScaleLevels):\n";
		Script = Script + "\tAtlasBCommand = AtlasBCommand + \" --scaleLevel \" + str(ScaleLevels[scale]) + \" --numberOfIterations \" + str(NbIter[scale])\n"; // ""
	...
	Script = Script + "if Overwrite==1 or not os.path.isfile(FinalAtlasPrefix + str( len(ScaleLevels)-1 ) + \".mhd\") :\n";
		...
		Script = Script + "\twhile case < len(allcases):\n";
			...
			Script = Script + "\t\tcase += 1\n";
	Script = Script + "else : print(\"=> The file \\'\" + FinalAtlasPrefix + str( len(ScaleLevels)-1 ) + \".mhd\\' already exists so the command will not be executed\")\n\n";
...
void ScriptWriter::setScaleLevels(std::vector< std::vector<int> > AtlasWerksScaleLevels)
{
